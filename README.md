...
$$\mathrm{VaR}\sb{\alpha} = \inf\,\lbrace\,l \in \mathbb{R} : P(L > l) \leq 1 - \alpha\,\rbrace$$

### Expected Shortfall (ES / CVaR)

The mean loss conditional on exceeding VaR:

$$\mathrm{ES}\sb{\alpha} = \mathbb{E}[L \mid L > \mathrm{VaR}\sb{\alpha}]$$
...
$$\sigma\sb{t}\sp{2} = \omega + \alpha \varepsilon\sb{t-1}\sp{2} + \beta \sigma\sb{t-1}\sp{2}$$
...
- **GARCH and EGARCH** — EGARCH captures the leverage effect (negative returns increase volatility more than positive returns of equal size). The EGARCH(1,1) specification:
  ...
  $$\ln\sigma\sb{t}\sp{2} = \omega + \alpha\left|\frac{\varepsilon\sb{t-1}}{\sigma\sb{t-1}}\right| + \gamma\frac{\varepsilon\sb{t-1}}{\sigma\sb{t-1}} + \beta\ln\sigma\sb{t-1}\sp{2}$$
...
$$\mathrm{VaR}\sb{\alpha} = \operatorname{Percentile}\bigl(\lbrace r\sb{t} \rbrace\sb{t=1}\sp{T},\; 1-\alpha\bigr)$$
...
$$\mathrm{VaR}\sb{\alpha} = \mu + \sigma \cdot z\sb{\alpha}$$
...
$$\mathrm{ES}\sb{\alpha} = \mu - \frac{\phi(z\sb{\alpha})}{1-\alpha} \cdot \sigma \quad \text{(Normal)}$$
...
$$S\sb{T} = S\sb{0} \cdot \exp\left[(\mu - \tfrac{1}{2}\sigma\sp{2})T + \sigma\sqrt{T} \cdot Z\right], \quad Z \sim \mathcal{N}(0,1)$$
...
$$LR\sb{\mathrm{POF}} = -2\ln\left(\frac{(1-\alpha)\sp{x}\,\alpha\sp{\,T-x}}{(x/T)\sp{x}\,(1-x/T)\sp{T-x}}\right) \;\sim\; \chi\sp{2}\sb{(1)}$$
...
$$LR\sb{\mathrm{Ind}} = -2\ln\left(\frac{(1-\pi)\sp{n\sb{00}+n\sb{10}}\,\pi\sp{n\sb{01}+n\sb{11}}}{(1-\pi\sb{0})\sp{n\sb{00}}\,\pi\sb{0}\sp{n\sb{01}}\,(1-\pi\sb{1})\sp{n\sb{10}}\,\pi\sb{1}\sp{n\sb{11}}}\right) \;\sim\; \chi\sp{2}\sb{(1)}$$
...
$$LR\sb{\mathrm{CC}} = LR\sb{\mathrm{POF}} + LR\sb{\mathrm{Ind}} \;\sim\; \chi\sp{2}\sb{(2)}$$
...
$$Z\sb{2} = \frac{1}{n}\sum\sb{t=1}\sp{n} \frac{R\sb{t}}{\mathrm{ES}\sb{t}} \cdot \mathbf{1}\sb{\lbrace R\sb{t} \leq \mathrm{VaR}\sb{t} \rbrace} + 1$$
